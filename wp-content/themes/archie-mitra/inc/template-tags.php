...
if ( ! function_exists( 'archie_posted_on' ) ) :
	/**
	...
	function archie_posted_on( $id = '' ) {
		...
		$time_string = '<time class="entry-date published updated" datetime="%1$s"><span class="date">%2$s</span><span class="month">%3$s</span></time>';
		if ( get_the_time( 'U', $id ) !== get_the_modified_time( 'U', $id ) ) {
			$time_string = '<time class="entry-date published" datetime="%1$s"><span class="date">%2$s</span><span class="month">%3$s</span></time><time class="updated" datetime="%4$s">%5$s</time>';
		}
		...
		printf(
			...
			__( '<span class="posted-on"><span class="screen-reader-text">Posted on</span> %s', 'archie' ),
